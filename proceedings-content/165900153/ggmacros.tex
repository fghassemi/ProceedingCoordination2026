...
\newcommand{\gsubs}[2]{^{#1} / _{#2}}
\newcommandx{\gsubst}[3][1=\aM,2=q,3=q',usedefault=@]{
  \left \{\gsubs{#3}{#2} \right \}#1
}
\newcommandx{\gsubsts}[5][1=\aM,2=q,3=q',4=q,5=q',usedefault=@]{
  \left \{\gsubs{#3}{#2}, \gsubs{#5}{#4} \right \}#1
}

\newcommandx{\isgc}[2][1=/tmp/__gc__]{
  \makeatletter\def\@gcpar{-o "#1" -gc "#2"}\makeatother
  \immediate\write18{gc2latex \@gcpar}
  \input{#1.tex}
}

\newcommandx{\isgcfile}[2][1=/tmp/__gc__]{
  \makeatletter\def\@gcpar{-o "#1" "#2"}\makeatother
  \immediate\write18{gc2latex \@gcpar}
  \input{#1.tex}
}
...
\newcommand{\gconst}[1]{\textsf{\emph{#1}}}

\newcommand{\gfparam}[1]{\textsf{\colorFparam #1}}
...
\def\colorPtp{\color{black}}
\def\colorFparam{\color{teal}}
...
%%% Chosem
\newcommandx{\wellpar}[2][1={\aG},2={\aG'},usedefault=@]{
  \mathit{wf}({#1}, {#2})
}
\newcommand{\extension}{\otimes}
...
\newcommand{\toolid}[1]{\textcolor{\toolidcol}{{\textsf{#1}}}\xspace}
\newcommand{\chortest}{\toolid{ChorTest}}
\newcommand{\fwkid}{\textsf{CGT}\xspace}

%%% Terminology
\newcommand{\lmutagens}[1][s]{l-mutagen{#1}\xspace}
\newcommand{\gmutagens}[1][s]{g-mutagen{#1}\xspace}
...
\newcommand{\chorgramsite}{\url{https://bitbucket.org/eMgssi/chorgram/src/master/}}
\newcommand{\chorgram}{\toolid{ChorGram}}

\newcommand{\gghide}[2]{{#1} \setminus {#2}}

\newcommand{\msgset}{\mathcal{\colorMsg{M}}}
\newcommand{\msgloop}[1][\cinit]{\msg[\lambda_{#1}]}
\newcommand{\msgexit}[1][\cfinal]{\msg[\epsilon_{#1}]}
...
\newcommand{\amsg}[1][m]{\ensuremath{\mathsf{\colorMsg{#1}}}\xspace}
\newcommand{\msg}[1][m]{\amsg[{#1}]}
\newcommand{\ptp}[1][A]{\ensuremath{\mathsf{\colorPtp{\capitalisewords{#1}}}}\xspace} % \newcommand{\ptp}[1][p]{\ensuremath{\mathsf{\colorPtp{\lowercase{#1}}}}\xspace}
\newcommand{\aptp}[1]{\ptp[#1]}
\newcommand{\p}{\ptp[a]\xspace}
\newcommand{\q}{{\ptp[B]}\xspace}
\newcommand{\s}{{\ptp[S]}\xspace}
\newcommand{\rr}{{\ptp[R]}\xspace}
\newcommand{\px}{{\ptp[x]}\xspace}
\newcommand{\py}{{\ptp[y]}\xspace}
\newcommand{\ps}{{\ptp[S]}\xspace}
\newcommand{\pr}{{\ptp[R]}\xspace}
...
\newcommand{\sndint}[1][{\gint[]}]{\mathrm{snd}(#1)}
...
\newcommand{\reccall}[2]{\gtvar{#1} \cdot {#2}}
% \newcommand{\gtvar}[1]{\dgt{\mathbf{#1}}}
% \newcommand{\dgt}[1]{\withcolor{\colorgp}{#1}}
\newcommand{\varid}[1][v]{\textcolor{MediumBlue}{\textsf{#1}}}
\newcommand{\withcolor}[2]{#2}
\newcommand{\truek}{\top}
\newcommand{\recctx}{\rho}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%%                        END CHOR MACROS                                %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\tikzset{
...
\newcommandx{\testsys}[2][1=\aM,2=\atestcase,usedefault=@]{#1 \otimes  #2}
\newcommand{\atestcase}[1][T]{#1}
\newcommand{\qsucc}[1][q]{\underline{#1}}
\newcommand{\Qsucc}[1][Q]{\underline{#1}}
\newcommand{\lineariz}[1][\p]{\ell_{#1}}
\newcommand{\normalform}{\square}
\newcommand{\exsem}[1][]{\gsem[\aG_\eqref{#1}]}
\newcommand{\pomsetTerm}[1][\ptpset']{\mathcal{R}_{#1}}
\newcommand{\CCP}[1][2]{\CC[{#1}-POM]}
\newcommand{\CC}[1][2]{\textsf{\textbf{CC{#1}}}}
\newcommand{\cfsmtr}[1][]{\xrightarrow{#1}}

\newcommandx{\aCFSM}[3][1=\aQ,2=\aQzero,3=\aTrs]{(#1,#2,#3)}
\newcommand{\cstr}[1]{{\xtwoheadrightarrow{\raisebox{-.3ex}[0pt][0pt]{$\scriptstyle #1$} }}}
\newcommand{\mkkeyword}[1]{\textcolor{\keywordcol}{\textsf{#1}}}

\newcommand{\pomtool}{\toolid{PomCho}}
\newcommand{\atree}{\tau}
\newcommand{\arun}{\pi}
\newcommandx{\aQfinal}[1][1=,usedefault=@]{
  {\ifempty{#1}{F}{F_{#1}}}
}
\newcommand{\testcompCh}[3]{#1 \triangleright_{#3} #2}
\newcommand{\testcompAll}[2]{#1 \triangleright #2}
\newcommand{\merged}{\otimes}
\newcommand{\runs}[1]{\Pi\left(#1\right)}
\newcommand{\aRun}{\pi}
\newcommand{\aCUT}{\ptp[A]}
\newcommand{\aProj}{\downharpoonright}
\newcommand{\aTestSystem}{\aG\aProj_{\aCUT} \merged \atestcase}
\newcommand{\aState}{\mathsf{q}}
\newcommand{\projword}[2]{{#1}\!\downharpoonleft_{#2}}
...
\newcommand{\ggclasstest}[1][]{
  \[
	 \begin{array}{c@{\hspace{3cm}}c}
		\begin{tikzpicture}[node distance=.6cm and 1cm, scale = .8, every node/.style={transform shape}]
		  \node at (0,0) (s1) {};
		  \node[right = 1.5cm of s1] (s2) {};
		  \mkint{below = of s1}{abx}[][@][int];
		  \mkint{below = of abx}{bcz}[][b][string][c];
		  \mkint{below = of s2}{acy}[][@][string][c];
		  \mkint{below = of acy}{cbw}[][c][int][b];
		  \mkseq{abx}{bcz};
		  \mkseq{acy}{cbw};
		  \mkbranch{abx,acy}[branch][][-.4];
		  \mkmerge{bcz,cbw}[merge][][.5];
		  \mkgraph{branch}{merge};
		\end{tikzpicture}
		&
		\begin{tikzpicture}[node distance=.6cm and 1cm, scale = .8, every node/.style={transform shape}]
		  \node at (0,0) (choice) {};
		  \mkint{}{abx}[][@][int];
		  \mkint{right = of abx}{cbx}[][c][int];
		  \mkbranch{abx,cbx}[par][];
		  \mkmerge{abx,cbx}[join][];
		  \mkint{left = of abx}{aby}[][@][string];
		  \mkint{below = of aby}{cby}[][c][string];
		  \mkseq{aby}{cby};
		  \mkfork{par, aby}[branch][][1.1];
		  \mkjoin{cby,join}[merge][][1];
		  \mkgraph{branch}{merge};
		\end{tikzpicture}
		\ifempty{#1}{}{
		\\\\\pause
...
\newcommandx{\synccfsmanimation}[3][1=a,2=c,3=b,usedefault=@]{
  \begin{overlayarea}{\textwidth}{\textheight}\vspace{-.5cm}
  \begin{block}{Internal step: $\aCS \trans \emptyword \aCS'$}\centering
	 \begin{tikzpicture}[mycfsm]
		\node[machinecloud, initial, initial text={\ptp[{#1}]}, initial distance = .3cm] (a) {$\bullet\phantom{\trans{\aout[#1][#2][]} \circ}$};
		\node[right = of a](dots1){$\cdots$};
		\node<1>[machinecloud, fill=cyan!10, initial, initial text={\ptp[{#2}]}, right = of dots1, initial distance = .3cm] (c) {$\bullet \trans \emptyword \circ$};
		\node<2->[machinecloud, initial, initial text={\ptp[{#2}]}, right = of dots1, initial distance = .3cm] (c) {$\circ \trans \emptyword \textcolor{orange}{\bullet}$};
		\node[right = of c](dots2){$\cdots$};
		\node[machinecloud, initial, initial text={\ptp[{#3}]}, right = of dots2, initial distance = .3cm] (b) {$\bullet\phantom{\trans{\ain[#1][#2][]} \circ}$};
	 \end{tikzpicture}
  \end{block}
  \begin{block}<3->{Interaction: $\aCS \trans {\gint[][#1][@][#3]}  \aCS'$}\centering
	 \begin{tikzpicture}[mycfsm]
		\node<3>[machinecloud, fill=cyan!10, initial, initial text={#1}, initial distance = .3cm] (a) {$\bullet \trans{\aout[#1][#3][]} \circ$};
		\node<4->[machinecloud, initial, initial text={#1}, initial distance = .3cm] (a) {$\circ \trans \aout \textcolor{orange}{\bullet}$};
		\node[right = of a](dots1){$\cdots$};
		\node[machinecloud, initial, initial text={#2}, right = of dots1, initial distance = .3cm] (c) {$\bullet$\phantom{$\trans \emptyword \circ$}};
		\node[right = of c](dots2){$\cdots$};
		\node<3>[machinecloud, fill=cyan!10, initial, initial text={#3}, right = of dots2, initial distance = .3cm] (b) {$\bullet \trans{\ain[#1][#3][]} \circ$};
		\node<4->[machinecloud, initial, initial text={#3}, right = of dots2, initial distance = .3cm] (b) {$\circ \trans{\ain[#1][#3][]} \textcolor{orange}{\bullet}$};
	 \end{tikzpicture}
  \end{block}
\end{overlayarea}
  \transdissolve<2>
...
\newcommand{\cfsmanimation}{
  ...
  \begin{center}
    \begin{tabular}{c@{\hspace{.1cm}}c@{\hspace{.1cm}}c}
      \hspace{-1cm}
      \begin{tikzpicture}[mycfsm, node distance = 3cm, initial where=left, scale=.7]
        ...
        %
		  \draw<2-8>[line width=.6ex, color=\newgreen!30, opacity=.2] (A0) -- (A1);
		  \draw<3-8>[line width=.6ex, color=\newgreen!30, opacity=.2] (A1) to[bend right] (A3);
        \path
        (A0) edge[sloped] node[cfsmlab] (a01) {$\aout[@][@][][int]$} (A1)
        (A0) edge[sloped] node[cfsmlab] {$\aout[@][@][][bool]$} (A2)
        (A1) edge[sloped, bend left] node[cfsmlab] {$\aout[@][@][][int]$} (A3)
        (A1) edge[sloped, bend right,below] node[cfsmlab] (a13) {$\aout[@][@][][bool]$} (A3)
        (A2) edge[sloped] node[cfsmlab]  {$\aout[@][@][][str]$} (A3)
        % (A3) edge [loop below] node [below] {$\ain[\q][\p][][{\msg[bool]}]$} (A3)
        ;
		  \transdissolve<2>[direction=-90]
		  \transwipe<3>[direction=0]
      \end{tikzpicture}
      &
      \uncover<2->{
        \begin{tabular}{c}
      ...
      &
      \begin{tikzpicture}[mycfsm,initial where=right, node distance=3cm, scale=.7]
        ...
        %
		  \draw<4-8>[line width=.6ex, color=red!60, opacity=.2] (A0) -- (A1);
		  \draw<5-8>[line width=.6ex, color=red!60, opacity=.2] (A1) to[bend right] (A3);
		  \draw<6-8>[line width=.6ex, color=red!60, opacity=.2] (A3) to[loop below, looseness = 20] (A3);
        \path
        (A0) edge[sloped] node[cfsmlab] (b01) {$\ain[@][@][][int]$} (A1)
        (A0) edge[sloped] node[cfsmlab]  {$\ain[@][@][][bool]$} (A2)
        (A1) edge[sloped, bend left] node[cfsmlab] {$\ain[@][@][][str]$} (A3)
        (A1) edge[sloped, bend right] node[cfsmlab,below] (b13) {$\ain[@][@][][bool]$} (A3)
        (A2) edge node  {$\ain[@][@][][str]$} (A3)
        (A3) edge[sloped, loop below, looseness = 20] node[cfsmlab] (b33) [right] {$\aout[B][A][][bool]$} (A3)
        ;
		  \transwipe<4>[direction=270]
		  \transwipe<5>[direction=0]
		  \transwipe<6-8>[direction=90]
      \end{tikzpicture}
      \\
      $\p$ & & $\q$
    \end{tabular}
...
%  \node<3-8>[rectangle, fill=\newgreen, fit=(a13),inner sep=0pt,opacity=0.2] {};
  ...
  % \node<4->[rectangle, fill=red!60, fit=(b01),inner sep=0pt,opacity=0.2] {};
  \node<4-4>[rectangle, fill=red!60, fit=(aba),inner sep=0pt,opacity=0.2] {};
%  \node<5->[rectangle, fill=red!60, fit=(b13),inner sep=0pt,opacity=0.2] {};
  \node<5-5>[rectangle, fill=red!60, fit=(abb),inner sep=0pt,opacity=0.2] {};
%  \node<6->[rectangle, fill=red!60, fit=(b33),inner sep=0pt,opacity=0.2] {};
  \node<6->[rectangle, fill=red!60, fit=(baa),inner sep=0pt,opacity=0.2] {};
  % \node<8-8>[rectangle, fill=\newgreen, fit=(b33),inner sep=0pt,opacity=0.2] {};
  \node<7->[rectangle, fill=red!60, fit=(input),inner sep=0pt,opacity=0.2] {};
  \node<8->[rectangle, fill=red!60, fit=(ba8),inner sep=0pt,opacity=0.2] {};
  % \node<9->[rectangle, fill=red!60, fit=(ba9),inner sep=0pt,opacity=0.2] {};
  % \node<9->[rectangle, fill=red!60, fit=(ba9),inner sep=0pt,opacity=0.2] {};
  % \node<10->[rectangle, fill=red!60, fit=(ba10),inner sep=0pt,opacity=0.2] {};
  % \node<11->[rectangle, fill=red!60, fit=(ba11),inner sep=0pt,opacity=0.2] {};
  % \node<10-10>[rectangle, fill=red!60, fit=(b33),inner sep=0pt,opacity=0.2] {};
  % \node<11-11>[rectangle, fill=red!60, fit=(b33),inner sep=0pt,opacity=0.2] {};
\end{tikzpicture}
}%%%end cfsmanimation


\newcommandx{\choranimation}[5][1=1,2=1,3=,4=.7,5=,usedefault=@]{
  % #1: factor for the width of the overlay area
  % #2: factor for the height of the overlay area
  % #3: bottom up part if not empty
  % #4: scale
  % #5: shows synchrony/asynchrony if empty
  \begin{overlayarea}{#1\textwidth}{#2\textheight}
    \begin{tikzpicture}[
...
%      font=\large
      ...
      \node [choreo] (global){Choreography $\aG$ \\ global viewpoint};
      % 
      \node [choreo, below =of global] (type1) {$\aCM_1$ \\ Local viewpoint$_1$};
      \node [choreo, right =of type1] (typei) {$\aCM_i$ \\ Local viewpoint$_i$};
      \node [choreo, right =of typei] (typen) {$\aCM_n$ \\ Local viewpoint$_n$};
		%
      \node<+-> (synctxt) [left = of global] {\ifempty{#5}{\textcolor{NavyBlue}{\bf Synchrony}}{}};
      \node<.-> (asynctxt) [left = of type1] {\ifempty{#5}{\textcolor{NavyBlue}{\bf Asynchrony}}{}};
		\ifempty{#5}{\node<+-> (wf) [right = of typen, fill=blue!10, drop shadow] {spec,no code};}
		\ifempty{#5}{\node<+-> (nocode) [above right = of typen, fill=blue!10, drop shadow] {Well-formedness};}
      % 
      \path<+-> [bigar] (global) edge[sloped,above] node {\color{OliveGreen}Project} (type1);
      \path<.-> [bigar] (global) edge[sloped,above] node {\color{OliveGreen}Project} (typei);
      \path<.-> [bigar] (global) edge[sloped,above] node {\color{OliveGreen}Project} (typen);
		%
      \path[elli] (type1) -- (typei);
      \path[elli] (typei) -- (typen);
      % 
      \node<+-> [process, below=of type1] (proc1) {Component$_1$};
      \node<.-> [process, below=of typei] (proci) {Component$_i$};
      \node<.-> [process, below=of typen] (procn) {Component$_n$};
      %
      \path<.-> [bigar,->,dashed,gray] (type1) edge[sloped,above] node {Verify} (proc1);
      \path<.-> [bigar,->,dashed,gray] (typei) edge[sloped,above] node {Verify} (proci);
      \path<.-> [bigar,->,dashed,gray] (typen) edge[sloped,above] node {Verify} (procn);
      % 
      \path<.-> [elli] (proc1) -- (proci);
      \path<.-> [elli] (proci) -- (procn);
		\ifempty{#3}{}{
		  %
		  \node<+-> [process, right=of procn,xshift=4cm] (evolve1) {Component'$_1$};
		  \node<.-> [process, right=of evolve1] (evolvei) {Component'$_i$};
		  \node<.-> [process, right=of evolvei] (evolven) {Component'$_n$};
		  %
		  \path<.-> [bigar,blue,dotted] (procn) edge node [above] {evolve/replace/compose} (evolve1);      
        \path<.-> [elli] (evolve1) -- (evolvei);
        \path<.-> [elli] (evolvei) -- (evolven);
		  %
		  \node<+-> [choreo, above=of evolve1, yshift=1.5cm] (t11) {New $\aCM'_1$ \\ Local viewpoint$_1$};
		  \node<.-> [choreo, above=of evolvei, yshift=1.5cm] (t1i) {New $\aCM'_i$ \\ Local viewpoint$_i$};
		  \node<.-> [choreo, above=of evolven, yshift=1.5cm] (t1n) {New $\aCM'_n$ \\ Local viewpoint$_n$};
		  %
		  \path<.-> [elli] (t11) -- (t1i);
        \path<.-> [elli] (t1i) -- (t1n);
        %
        \path<.-> [bigar,->,dashed,gray] (evolve1) edge[sloped,above] node {Extract} (t11);
        \path<.-> [bigar,->,dashed,gray] (evolvei) edge[sloped,above] node {Extract} (t1i);
        \path<.-> [bigar,->,dashed,gray] (evolven) edge[sloped,above] node {Extract} (t1n);
		  %
		  \node<+> [above=of t1i, yshift=.5cm] (qm) {\Huge \textcolor{red}{ ??? }};
		  \node<.-> [above=of t1i] (dummy) {};
		  \node<+-> [choreo,above=of dummy, yshift=-.5cm] (global') {New choreography $\aG'$ \\ global viewpoint};
		  %
		  \path<.-> [bigar,-] (t11) -- (dummy);
        \path<.-> [bigar,->] (t1i) edge node[right,xshift=1em] {\color{OliveGreen}Synthesise} (global');
        \path<.-> [bigar,-] (t1n) -- (dummy);
		}
    \end{tikzpicture}
  \end{overlayarea}
}%%%end choranimation
...
\newcommand{\bracketColor}[1]{\textcolor{cyan!50!blue!80}{#1}}
\lstdefinelanguage{sgc}{
  commentstyle=\color{Gray},
  morecomment=[l]{..},
  morecomment=[n]{[[}{]]},
  keywords=[0]{repeat,branch,sel,let,in,with,unless},
  morekeywords=[1]{test},
  keywordstyle=[0]\color{teal}\sffamily,
  keywordstyle=[1]\color{red}\sffamily,
  mathescape=true,
  literate=
  {=}{{$\colorOp{=}$}}1
  {->}{{${\colorOp \xrightarrow{}}\ $}}2
  {=>}{{${\colorOp \Rightarrow{}}$}}2
  {:}{{$\colorOp{\colon}$}}1
  {|}{{$\gparop$}}1
  {;}{{$\gseqop$}}1
  {+}{{$\gchoop$}}1
...
\newcommand{\gparop}{{\colorOp \ \ensuremath{\mid}\ }}
...
%
\newcommand{\gkword}[1]{\textcolor{OliveGreen}{\ensuremath{\mathsf{#1}}}}
\newcommand{\gbrakw}{\gkword{branch}}
\newcommand{\gselkw}{\gkword{sel}}
\newcommand{\grepkw}{\gkword{repeat}}
\newcommand{\gletkw}{\gkword{let}}
\newcommand{\ginkw}{\gkword{in}}
\newcommand{\gdokw}{\gkword{do}}
\newcommand{\gwithkw}{\gkword{with}}
\newcommand{\gblk}[1][\aG]{\textcolor{NavyBlue}{\{} #1 \textcolor{NavyBlue}{\}}}
\newcommand{\gsel}[1]{\gselkw\ \gblk[\dolist{\gchoop}{#1}]}
\newcommand{\gseql}[1]{\dolist{\gseqop}{#1}}
\newcommand{\gparl}[1]{\dolist{\gparop}{#1}}
\newcommand{\glet}[2]{
  \ifempty{#1}{#2}{\gletkw\ {#1} \ginkw\ {#2}}
}
...
\makeatletter
\def\mklogtype#1{
  \def\nextitem{\def\nextitem{\logcat\penalty0}}%
  \@for\el:=#1\do{\nextitem\aeventtype[][{\el}]}%
}
...
\def\mksys#1{
  \def\transform##1[##2]{(\afish[][##1],\mklog{##2})\parop\penalty0}%
  \@for\el:=#1\do{\expandafter\transform\el}%
}
\makeatother

\newcommand{\cpset}{\mathbb{\colorNode{Z}}_0}
\newcommand{\nodeset}{\cpset}
\newcommand{\ghole}{{\colorOp$\square$}}
% \newcommand{\gempty}{\textsf{0}}
\newcommand{\gempty}{\ensuremath{\circledcirc}}%{\textsf{(o)}}
\newcommandx{\refgint}[3][1=A,2=\msg,3=B,usedefault=@]{
  \ptp[#1] {\colorOp \xdashrightarrow[{}]{\msg[{#2}]}}{
    \renewcommand{\do}[1]{\ptp[##1]}
	 \docsvlist{#3}
  }
}
...
\tikzset{
  gt/.style={
         ...
         node distance=3cm,
         scale = .5,
         every state/.style={inner sep = 2pt, minimum size = 0pt, font=\footnotesize, transform shape},
         transform shape
  },
  % gt/.style={
...
\newcommandx{\mkhg}[5][1={},4={},5={},usedefault=@]{
  \begingroup
  \tikzset{#1}
  \StrCount{#2,}{,}[\l] % from package xstring
  \StrCount{#3,}{,}[\m] % from package xstring
...
\newcommand{\clang}{\ensuremath{\mathcal{\colorLang L}}}
\newcommandx{\glang}[1][1=\aG,usedefault=@]{\mathcal{\colorLang \widehat{L}}[#1]}
\newcommand{\alang}{\ensuremath{\mathbb{\colorLang L}}}
...
\newcommand{\pomset}[2][.8]{
  \left[
	 \begin{tikzpicture}[pomset, scale = #1]
		% Pomset
		#2
	 \end{tikzpicture}
  \right]
...
\newcommandx{\mkfork}[5][2=gatenode,3=i,4=.6,5=2,usedefault=@]{
  \mkgatebegin{#1}[{\gname[{#3}]}][agate][#4]{#2}[#5]
}
% \newcommandx{\mkfork}[5][2=gatenode, 3=i, 4=.6, 5=2, usedefault=@]{
%   \mkgatebegin{#1}[{\gname[{#3}]}][agate][#4]{#2}[#5]
% }

\newcommandx{\mkbranch}[5][2=gatenode, 3=i, 4=.6, 5=2, usedefault=@]{
  \mkgatebegin{#1}[{\gname[{#3}]}][ogate][#4]{#2}[#5]
}

\newcommandx{\mkgatebegin}[6][2={}, 3=ogate, 4=.5, 6=2, usedefault=@]{
  % #1: list of nodes
  % #2: control point
  % #3: gate type
  % #4: vertical position offset
  % #5: name of the gate node
  % #6 tolerance of |- lines
  %
  \coordinate (gatecoord) at (0,0);
  ...
  \coordinate (gatecoord) at ($(xmin)!.5!(xmax) + (max) + (0,#4) + (max)$);
  \node[#3,label={below:$#2$}] (#5) at (gatecoord) {};
  \pgfgetlastxy{\xgate}{\ygate};
  \pgfmathtruncatemacro{\xgateround}{\xgate};
  \StrCount{#1,}{,}[\l] % from package xstring
  ...
  \else{
    \foreach \n in {#1}{
      ...
      \pgfmathtruncatemacro{\xnround}{\xnode};
      \pgfmathsetmacro\tmpdiff{abs(\xnround - \xgateround)}
      \ifdim \tmpdiff pt > #6 pt {\path[line] (#5) -| (\n);}
      \else
        \path[line] (#5) -- (\n);
      \fi
    }
...
\newcommandx{\mkgatebeginold}[5][2={},3=ogate,4=.5,usedefault=@]{
  ...
  \foreach \n [count=\i] in {#1}{
    ...
    \path (\n);
    \pgfgetlastxy \xn \yn;
    \coordinate (gatecoord) at ($(gatecoord) + (\xn,0)$);
    \coordinate (gatecoord) at ($1/\i*(gatecoord)$);
    \ifdim \yn < \yc
    \node (max) at (0,\yc) {};
    \else
    \node (max) at (0,\yn) {};
  ...
  \coordinate (gatecoord) at ($(gatecoord) + (0,#4) + (max)$);
  \node[#3,label={below:$#2$}] (#5) at (gatecoord) {};
  \pgfgetlastxy{\xgate}{\ygate};
  \pgfmathtruncatemacro{\xgateround}{\xgate};
  \StrCount{#1,}{,}[\l] % from package xstring
...
\newcommandx{\mkmerge}[5][2=gatenode,3=i,4=.5,5=2,usedefault=@]{
  \mkgateend{#1}[{\ifempty{#3}{}{\nmerge[#3]}}][ogate][#4]{#2}[#5]
}

\newcommandx{\mkjoin}[5][2=gatenode,3=i,4=.5,5=2,usedefault=@]{
  \mkgateend{#1}[{\ifempty{#3}{}{\nmerge[#3]}}][agate][#4]{#2}[#5]
}

\newcommandx{\mkgateend}[6][2={},3=ogate,4=.5,6=2,usedefault=@]{
  % #1 list of nodes
  % #2 control point
  % #3 gate type
  % #4 vertical position offset
  ...
  \coordinate (gatecoord) at ($(xmin)!.5!(xmax) + (ymin)$);
  \node[#3,label={above:$#2$}] (#5) at ($(gatecoord) - (0,{#4})$) {};
  \pgfgetlastxy{\xgate}{\ygate};
  \pgfmathtruncatemacro{\xgateround}{\xgate};
  \StrCount{#1,}{,}[\l] % from package xstring
  \ifnum \l < 2 {\errmessage{#1 argument should be a comma-separated list of lenght >= 2}}
  \else{
    \foreach \n in {#1}{
      ...
      \pgfmathtruncatemacro{\xnround}{\xnode};
      \pgfmathsetmacro\tmpdiff{abs(\xnround - \xgateround)}
      \ifdim \tmpdiff pt > #6 pt {\path[line] (\n) |- (#5);}
      \else
        \path[line] (\n) -- (#5);
      \fi
    }
...
\tikzset{
  ...
  process/.style={rectangle, fill=\fillcolor, drop shadow, align=center,
    text centered, text=gray%, minimum height=4em
  },
  choreo/.style={rectangle, fill=\fillcolor, drop shadow, align=center,
    text centered, rounded corners%, minimum height=2em%, minimum width=13em
  },
  machinecloud/.style={
...
\tikzset{
  mycallout/.style={
	 fill=gray!30, opacity=.5, overlay, align=center,
	 cloud callout, cloud puffs=10, aspect=1.9, cloud ignores aspect, cloud puff arc=100
  }
}

\newcommandx{\ggvisually}[8][1=5pt,2=15pt,3=5pt,4=5pt,5=1.0cm,6=\scriptsize,7={},8={},usedefault=@]{
  % par 7: iteration
  % par 8: caption
  \def\dist{\hspace{#5}}
...
\newcommandx{\newggvisually}[5][1=5pt,2=15pt,3=\scriptsize,4={},5={},usedefault=@]{
  % par 4: iteration
  % par 5: caption
  % gempty
  \def\w{1cm}
  \begin{minipage}[c]{\w}
	 \ifempty{#5}{}{\text{#3 empty}\\[#1]}
	 \begin{tikzpicture}[node distance=0.9cm and 0.4cm, every node/.style={scale=.7,transform shape}]
		\node[source] (srcint) {};
		\node[sink,below=of srcint] (sinkint) {};
		\node[mycallout, above = .3cm of srcint, xshift=1cm, callout absolute pointer={(srcint.east)}] {source node};
  ...
  \hfill
     ...
     \ifempty{#5}{}{\text{#3 parallel}\\[#1]}
		 \begin{tikzpicture}[node distance=.4cm and 0.4cm, every node/.style={scale=.7,transform shape}]
			...
			\mkjoin{j,jp}[join][][#2];
			\mkgraph{fork}{join};
			\node[mycallout, above = .2cm of fork, xshift=-1cm, callout absolute pointer={(fork.west)}] {#3 fork gate};
			\node[mycallout, below = .2cm of join, xshift=-1cm, callout absolute pointer={(join.west)}] {#3 join gate};
		 \end{tikzpicture}
	  \end{minipage}
	  \hfill
	  % gcho
	  \begin{minipage}[c]{\w}
     \ifempty{#5}{}{\text{#3 branch}\\[#1]}
		 \begin{tikzpicture}[node distance=.4cm and 0.4cm, every node/.style={scale=.7,transform shape}]
			\node[bblock] at (-.7,0) (g) {$\aG$};
			\node[bblock] at (.7,0)  (gp) {$\aG'$};
			\node[node, above=of g] (f) {};
			...
			\path[line,dotted] (fp) -- (gp);
			\path[line,dotted] (gp) -- (jp);
			\mkbranch{f,fp}[fork][][#1];
			\mkmerge{j,jp}[join][][#2];
         \mkgraph{fork}{join};
         \node[mycallout, above = .1cm of fork, xshift=-1cm, callout absolute pointer={(fork.west)}] {branch gate};
         \node[mycallout, below = .1cm of join, xshift=-1cm, callout absolute pointer={(join.west)}] {merge gate};
       \end{tikzpicture}
	  \end{minipage}
     \ifempty{#4}{}{
     \hfill
     ...
     \ifempty{#5}{}{\text{#3 iteration}\\[#1]}
        ...
        \mkgraph[.3cm]{entryf}{exitj};
        \node[mycallout, above = .2cm of entryf, xshift=1.3cm, callout absolute pointer={(entryf.east)}] {loop entry};
        \node[mycallout, above = -.7cm of exitj, xshift=1.3cm, callout absolute pointer={(exitj.east)}] {loop exit};
      \end{tikzpicture}
	  \end{minipage}
     }
}
  ...
  \newcommandx{\wwwcquote}[1][1=quo:w3c,usedefault=@]{
	 \ifempty{#1}{}{\begin{quote}\label{#1}}\renewcommand{\baselinestretch}{-.15}
		...
		resulting \textcolor{orange}{local systems} [...]\rq\rq
		\ifempty{#1}{}{\end{quote}}
  }
