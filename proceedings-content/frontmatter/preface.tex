...
This volume contains the proceedings of the \textbf{28th International Conference on Coordination Models and Languages (COORDINATION 2026)}, held on June 8–12, 	 2026, in Urbino (Italy), hosted by the
...
The COORDINATION conference series 
 has a long history of contributions
 related to the study and engineering of coordinated systems.
%
Interaction is a fundamental dimension in systems.
%
Modern information systems increasingly rely on combining concurrent, distributed,
mobile, adaptive, reconfigurable, and heterogeneous components. 
%
New models, architectures, languages, and verification techniques are necessary to cope with the complexity induced by the demands of today’s software development. 
%
...
COORDINATION 2026 solicited contributions in three different categories: (1) \emph{regular papers} describing thorough and complete research results and experience reports;
(2) \emph{survey papers} describing important results and success stories related to
the topics of COORDINATION; and (3) \emph{tool papers} describing technological artifacts
...
done electronically, in two phases. In the first phase, %which lasted three weeks, 
 each
submission was single-blind reviewed by at least three PC members, in some cases with
the help of external reviewers. During the second phase, %which lasted four days, 
the papers were thoroughly discussed. 
%
The decision to accept or reject a paper
was based not only on the reviews and scores but also on these in-depth discussions. 
%
In
...
\item The paper ``Timed Scenario Expressions and Realisability''  %addresses the problem of realisability for the distributed system modeled by sets of sequential timed scenarios.  
  introduces the notion of timed scenarios expressions to tackle the problem of realisability of global models of distributed systems denoted as sets of sequential timed scenarios.
...
\item The third paper ``Motif Refinement for the Hierarchical Control of Structured CPSs'' introduces a refinement-based modeling and development approach for cyber-physical systems which preserves control stability.  
\end{itemize}

\paragraph{Distributed algorithms for collective adaptive systems.} 
This group contains two papers which propose collective distributed algorithms within the aggregate programming framework with specific guarantees. 
\begin{itemize}
\item The paper ``Aggregate Indoor Localisation" reformulates existing localisation algorithms in aggregate computing settings, provides stabilisation guarantees, and propose improved algorithmic variants %to guarantees  adaptability and resilience while improving their performance.
   that are experimentally studied w.r.t. resilience and performance.
\item The paper ``A Self-Stabilizing Min-Max Consensus via Path-loop Detection" proposes a novel min-max consensus algorithm, proves its self-stabilization property, and experimentally assess its Collektive implementation for correctness and resilience by simulation. 
\end{itemize}
...
\item The paper ``Runtime adaptation as a programming pattern in service composition'' addresses the dynamic reconfiguration of service-oriented applications by proposing an adaptation pattern leveraging behavioural contracts and semantic-based dynamic discovery and binding of services.
\item The tool paper ``Phyelds: A Pythonic Framework for Aggregate Computing'' provides a Python library for aggregate programming, paving the path for integration of this collective computing paradigm in the Python and machine learning tool ecosystem.
\end{itemize}
...
Furthermore, as Programme Chairs, we actively contributed to the selection of the three keynote
...
Organizing Committee, chaired by Claudio Antares Mezzina, for their effort in organizing the conference. We also thank
...
Gul Agha & University of Illinois at Urbana Champaign, United States of America\\
...
Christine Julien & University of Texas at Austin, United States of America\\
...
\noindent
\begin{longtable}{p{0.42\textwidth}p{0.58\textwidth}}
